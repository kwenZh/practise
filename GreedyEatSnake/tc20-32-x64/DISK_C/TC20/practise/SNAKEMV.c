...
void showMapHead(SNAKE *mySnake) {
	...
	printf("LENGTH-[%d]\t\tScore-[%d]\t\tLEVEL:[%d ms]",
		mySnake->snake_length, (mySnake->snake_length-2)*5, mySnake->game_level / 20);
}
...
void showRandomFood(SNAKE *mySnake) {
	...
	mySnake->food.y = allPoint[random_index ] / MAX_X + 1;
...
void showOver(int score) {
	gotoxy(30, 12);
	printf("GAME GG! YOUR SCORE = %d", score);
}
...
void beginMoveSnake(SNAKE *mySnake) {
	...
	while(keyIn != ESC && mySnake->isAlive && mySnake->snake_length <= MAX_LENGTH ) {
		if(bioskey(1)) {
			...
			direct = (!mySnake->snake_is_run || (abs(getPointDirectByKey(keyIn) - direct) == 2) || getPointDirectByKey(keyIn) == KEY_ERROR ) 
